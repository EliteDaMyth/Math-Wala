...
let parseSubpods = (pod) => {
	...
	return subpods.map((subpod) =>
		Array.from(subpod.getElementsByTagName("img")).map((img) => {
					...
					z(
						{
							...
							onclick(e) {
								...
								sbso.className =
									sbso.className === "hidden" ? "shown" : "hidden";
								z.update();
							},
						},
	...
	z.mgl20(
		z._input.query({
			...
			onkeydown(e) {
				if (e.key === "Enter") {
					e.preventDefault();
					req(e.target.value).then((e) =>
						e
							.text()
							.then((e) => (pods = processPods(e)))
							.then(z.update)
					);
				}
			},
		...
		(e) =>
			pods.map((pod) =>
				z.pod(
					{
						on$created(e) {
							pods_objs.push(e.target);
						},
						id: pod.getAttribute("id"),
					},
					[
						(e) =>
							z.mgr18.aqb.fs16.mgt10.mgb10(`${pod.getAttribute("title")}:`),
