import { APPID, SIG_SALT, WA_SERVER } from "./config.js";
function md5(d) {
...
function calculate_signature(query) {
	let params = query
		.split("&")
		.map((x) => x.split("="))
		.filter((x) => x.length > 1);
...
function craft_signed_url(query) {
	...
	for (var [key, value] of _query.entries())
		query.push(`${encodeData(key)}=${encodeData(value)}`);
	query.sort();
	query = query.join("&");
	console.log(query);
	query = query + `&sig=${calculate_signature(query)}`;
	console.log(query);

	return `https://${WA_SERVER}/v2/query.jsp?${query}`;
}

export function wa_request(query, params = null) {
	return fetch(craft_signed_url(query));
}

export let req = (q) =>
	wa_request(`input=${encodeData(q)}&podstate=Step-by-step%20solution`);
